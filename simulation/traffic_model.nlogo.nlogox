...
to setup-cars
  create-turtles num-cars [
    ...
    move-to one-of patches with [road? and not intersection?]
...
to update-traffic-lights
  ...
  if light-timer > 30 [
    ask patches with [traffic-light = "red"] [
      set traffic-light "green"
      set pcolor lime
    ]

    ask patches with [traffic-light = "green"] [
      set traffic-light "red"
      set pcolor red
    ]
...
to move-car
  ...
  ;; stay only on roads
  if [road?] of next-patch = false [
    rt 90
    stop
  ]

  ;; avoid crashing into nearby cars
  if any? other turtles-on next-patch [
    set stopped? true
    stop
  ]

  ;; stop near red light
  if [traffic-light] of next-patch = "red" [
    set stopped? true
    stop
  ]
...
to calculate-statistics
  set waiting-cars count turtles with [stopped?]
  set congestion (waiting-cars / count turtles) * 100
  set average-speed mean [speed] of turtles
end
to export-data
  file-open "../dataset/traffic_data.csv"

  if ticks = 0 [
    file-print "tick,num_cars,waiting_cars,congestion,average_speed"
  ]

  file-print (word ticks "," count turtles "," waiting-cars "," congestion "," average-speed)

  file-close
end]]></code>
  <widgets>
    <view x="368" wrappingAllowedX="true" y="10" frameRate="30.0" minPycor="-16" height="434" showTickCounter="true" patchSize="13.0" fontSize="10" wrappingAllowedY="true" width="434" tickCounterLabel="ticks" maxPycor="16" updateMode="0" maxPxcor="16" minPxcor="-16"></view>
    <button x="125" y="20" height="40" disableUntilTicks="false" forever="false" kind="Observer" width="55" display="Setup">setup</button>
    <button x="160" y="150" height="40" disableUntilTicks="false" forever="true" kind="Observer" width="55" display="Go">go</button>
    <slider x="65" step="1" y="70" max="200" width="250" display="num-cars" height="50" min="10" direction="Horizontal" default="50.0" variable="num-cars"></slider>
    <monitor x="85" precision="17" y="235" height="60" fontSize="11" width="100" display="Cars">count turtles</monitor>
    <monitor x="205" precision="17" y="235" height="60" fontSize="11" width="100" display="Ticks">ticks</monitor>
    <monitor x="15" precision="17" y="310" height="60" fontSize="11" width="100" display="Congestion">congestion</monitor>
    <monitor x="125" precision="17" y="305" height="60" fontSize="11" width="100" display="Waiting-cars">waiting-cars</monitor>
    <monitor x="250" precision="17" y="300" height="60" fontSize="11" width="100" display="Average-speed">average-speed</monitor>
  </widgets>
  <info>## WHAT IS IT?
